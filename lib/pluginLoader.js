...
class pluginLoader {
    constructor() {
        ...
        this.watcher = {};
        this.task = []; //定时任务
        this.filemage = new Filemage(Path.qianyuPath + 'apps/');
    }
    ...
    async getConfig() {
        if (this.isYunzai()) {
            const configPath = path.resolve(__dirname, '../../../lib/config/config.js');
            if (!fs.existsSync(configPath)) {
                console.warn(`Config file ${configPath} does not exist.`);
                return {}; // 返回空对象以避免错误
            }
    ...
    filterGroup(e) {
        const filterGroup = Config.GetCfg('system/apps').filterGroup || [];
    ...
    async checkBlack(e) {
        const other = (await this.getConfig()).getOther();
